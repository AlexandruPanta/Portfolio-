...
/* Balises de partage, identiques pour la home et les case studies.
   ...
   identique dans copy.fr et copy.en pour une même page : le préfixe de
   langue est ajouté ici, pas dans le contenu.
...
export default function Meta({ title, description, path, ogImage }) {
  ...
  /* La locale par défaut de next.config.js n'a pas de préfixe — pas
     "fr" en dur : x-default doit suivre quelle que soit la locale
     choisie comme racine. */
  const withLocale = (loc) => (loc === defaultLocale ? path : `/${loc}${path}`);
  ...
  const urlDefault = resolved ? `${resolved}${withLocale(defaultLocale)}` : null;
  ...
  return (
    <Head>
      ...
      {urlDefault ? <link rel="alternate" hrefLang="x-default" href={urlDefault} /> : null}
