...
const probe = () => {
  const masked = [...document.querySelectorAll('[data-reveal],[data-reveal-lines],[data-trace]')]
    .filter((el) => {
      const s = getComputedStyle(el);
      const t = s.transform;
      return s.opacity !== '1' || (t !== 'none' && !t.startsWith('matrix(1, 0, 0, 1'));
    })
    .map((el) => (el.textContent || '(filet)').trim().slice(0, 40));
  return {
...
for (const path of PAGES) {
  ...
  const ok = emulated && worstMasked === 0 && worstAnim === 0 && !loaderSeen && text > 500;
  if (!ok) failed = true;

  console.log(`${ok ? '  OK  ' : '  ÉCHEC'} ${path}`);
  console.log(`         média émulé : ${emulated} · masqués : ${worstMasked} · animations : ${worstAnim} · loader vu : ${loaderSeen} · texte : ${text} car.`);
