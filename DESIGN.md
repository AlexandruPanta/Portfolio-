...
| (03) Contact | Bloc **pleine largeur** : l'adresse fait 1034px en `--t-h2` à 1440, elle ne tient pas dans huit colonnes. Adresse en `--accent`, statut et liens en mono. Téléphone (`tel:+33763077798`) en premier de `contact.links`, même `.contactLink` que LinkedIn/GitHub — sauf `target`/`rel`, sans objet pour un lien `tel:`. `white-space: nowrap` sur `.contactLink` : un numéro ne casse jamais au milieu. Identique fr/en, jamais traduit. Pas de formulaire. |
