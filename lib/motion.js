...
/**
 * Le nom du hero (V1.1, DESIGN.md §4) — Satoshi est variable, sa graisse
...
/**
 * Le moment fort du site (V1.1, DESIGN.md §4) — la chaîne d'architecture
...
/**
 * Transition de page (V1.1, DESIGN.md §4) — un filet 1px balaie
 * l'écran, puis la navigation a lieu, puis la page arrivée joue son
 * reveal existant sans rien de nouveau. Retour : même filet, sens
 * inverse. --ease et --d-base réutilisés, aucun nouveau réglage.
 *
 * Vit dans ce module — pas un fichier séparé — pour partager la même
 * instance GSAP déjà chargée par useSiteMotion : un second import()
 * dupliquerait GSAP dans un chunk distinct.
 *
 * Jamais au premier chargement : n'existe que sur clic. Le loader ne
 * rejoue jamais en navigation client, ce mécanisme n'y touche pas.
 */
let bar = null;
...
/**
 * @param router   instance next/router
 * @param href     destination
 * @param reverse  sens inverse (retour) : le filet part de la droite
 */
export function sweepTo(router, href, { reverse = false } = {}) {
  ...
  gsap.set(el, { x: reverse ? vw : 0, opacity: 1 });
  gsap.to(el, {
    x: reverse ? 0 : vw,
    duration,
    ease: 'site',
